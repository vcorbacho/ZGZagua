 ...
 class controller_user {
 	...
 	function login() {
	 	...
	 	if( $provider == 'yahoo' || $provider == 'google' ) {
			...
			$openid = new LightOpenID;
	 		...
	 		$openid->required = array('namePerson/friendly', 'contact/email', 'namePerson', 'identityProvider/userId');
 	 ...
 	 */
 	 private function user_login( $email = '', $external = false, $password = '' ) {
 	 	if( $email == '' ) $email = Controller::post('e-mail');
 	 	if( $password == '' ) $password = Controller::post('password');
 	 	
 	 	// Si external es true miramos si esta en la bdd
 	 	if ( $external ) {
 	 		$usuario = new model_user( $email, '', true );
 	 	} else {
 	 		$usuario = new model_user( $email, $password );
 	 	}
 	  ...
 	  */
 	  public function user_register( $mail = '', $external = false ) {
 	  	if( $mail == '' ) $mail = Controller::post('e-mail');
